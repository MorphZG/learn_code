...
PSQL="psql --username=zoran --dbname=worldcup --no-align --tuples-only -c"
...
echo "$($PSQL "SELECT teams.name FROM teams JOIN games ON teams.team_id = games.opponent_id OR teams.team_id = games.winner_id WHERE games.year = 2014 AND games.round = 'Eighth-Final' ORDER BY teams.name ASC")"
...
#tags: advanced queries, join, math function,