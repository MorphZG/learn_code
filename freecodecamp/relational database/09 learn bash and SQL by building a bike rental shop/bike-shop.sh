...
MAIN_MENU() {  # function definition
  if [[ $1 ]]   # if first argument is true
  then
    echo -e "\n$1"  # echo first argument
  fi

  echo "How may I help you?"  # function call without arguments 
  echo -e "\n1. Rent a bike\n2. Return a bike\n3. Exit"
  read MAIN_MENU_SELECTION  # read user input
...
RENT_MENU() {
  ...
  if [[ -z $AVAILABLE_BIKES ]]  # if $AVAILABLE_BIKES is false, empty variable
  then
    # send to main menu
    ...
    do
      echo "$BIKE_ID) $SIZE\" $TYPE Bike"  # print format: 1) 27" Mountain Bike
    done

    # ask for bike to rent
    echo -e "\nWhich one would you like to rent?"
    read BIKE_ID_TO_RENT  # read user input

    # if input is not a number
    if [[ ! $BIKE_ID_TO_RENT =~ ^[0-9]+$ ]]  # if not #~ regex test
    then
      # send to main menu
      ...
      # if not available
      if [[ -z $BIKE_AVAILABILITY ]]  # if $BIKE_AVAILABILITY is false, empty variable
      then
        # send to main menu
        MAIN_MENU "That bike is not available."  # call MAIN_MENU with string as arugment
      else
        # get customer info
        ...
        if [[ -z $CUSTOMER_NAME ]]  # IF $CUSTOMER_NAME is false, empty variable
        then
...
RETURN_MENU() {
  ...
  then
    # send to main menu
    MAIN_MENU "I could not find a record for that phone number."  # call MAIN_MENU with string as argument
  else
    ...
    then
      ...
      # if not a number
      if [[ ! $BIKE_ID_TO_RETURN =~ ^[0-9]+$ ]]  # if not #~ regex test
      then
        # send to main menu
        MAIN_MENU "That is not a valid bike number."
      else
...
# when everything is done, read the code from start to end,
# try to understand everything and add comments where needed.

#tags: function, while do loop, psql, 