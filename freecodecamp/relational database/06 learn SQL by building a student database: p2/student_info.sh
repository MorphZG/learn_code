...
# try to echo without first set of double quotes, will output in a single line
echo "$($PSQL "SELECT first_name, last_name, gpa FROM students WHERE gpa=4.0")"

echo -e "\nAll course names whose first letter is before 'D' in the alphabet:"
# can compare alphabet characters from ASCII
echo "$($PSQL "SELECT course FROM courses WHERE course < 'D';")"

# where multiple conditions OR multiple conditions
...
# sort in descending/ascending order and limit the output by number of results
...
echo -e "\nMajor ID, total number of students in a column named 'number_of_students', and average GPA rounded to two decimal places in a column name\
 'average_gpa', for each major ID in the students table having a student count greater than 1:\n"
...
# joining multiple tables in one. there are many available parameters on joining tables. google for more info.
...
#tags: bash, psql, comparison, filter query, select, where, condition, like pattern, regex, limit, sort, order, math, count, group, having, join